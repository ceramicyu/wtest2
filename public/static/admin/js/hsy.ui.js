// 幻灯片实现
function slideBanner(_container, _delay) {
		...
		this.updateBanner = function() {
			...
			for(var i = 0; i < pic_list.length; i++) {
				container.find(".banner_list").append('<div class="banner_item"><img src="'+pic_list[i].url+'" data-link="'+pic_list[i].link+'" title="'+pic_list[i].title+'" /></div>');
				container.find(".banner_tools").append('<a class="banner_anchor" data-id="'+i+'"></a>');
			}
			container.find(".banner_list").append('<div class="banner_item"><img src="'+pic_list[0].url+'" data-link="'+pic_list[0].link+'" title="'+pic_list[0].title+'" /></div>');
...
function pageTable(_container, _head, _body_format, _post_url, _param, _bindEvent, _count, _callprev, _search) {
	...
	this.getData = function() {
		...
		showLoading("正在载入...");
		AjaxPost(_post_url, $.extend(_param, {pageid: self.loadPage, first: first}), function(obj){
			...
			for(var k in _obj_list) {
				self.datalist.push(obj.data.list[k]);
			}
	...
	this.buildData = function() {
		...
		if (_end <= _begin) {
			hideLoading();
			_container.find(".ex_custom_table tbody").html("<tr><td colspan='100%'>暂无相关数据...</td></tr>");
			return;
		}

		// (优化方案2:) 将所有数据保存在内存中,当页码回滚的时候不再重新从数据库拉取,而应该从内存里获取
		// 判断是否需要从服务器拉取数据
		if (self.datalist.length < _end && self.datalist.length < self.totaldata) {
			self.getData();
			return;
		}
	...
	this.buildPageBtn = function() {
		_container.find(".ex_page_tools").html("<div class='ex_page_btn' data-id='prev'><</div>");
		...
		_container.find(".ex_page_tools").append("<div class='ex_page_btn' data-id='next'>></div>");
		self.bindEvent();
	}
